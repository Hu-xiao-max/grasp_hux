...
class grasp_config():
    def __init__(self, filepath):
            ...
            #生成1296个抓取向量
            self.grasp_vectors=self.generate_unit_sphere_points(6500)
            # 生成抓取坐标系
    ...
    def compute_contact(self, gripper_direction):
        ...
        # 提取可接触的点
        contact_points = np.asarray(self.pcd.points)[np.logical_and(np.radians(130) < angles, angles < np.radians(180))]
    ...
    def get_nearest_normal(self, query_point):
        """
        给定一个点，返回物体上离这个点最近的点的法向量。

        参数：
            query_point (tuple or list or numpy.ndarray): 查询点的坐标 (x, y, z)。

        返回：
            numpy.ndarray: 离查询点最近的点的法向量。
        """
